...
function frog_scene () {
    ...
    frog_width = 3;
    frog_height = 3;
    var frog_material = new THREE.MeshBasicMaterial( {color: 0x0A4F35, side: FrontSide} );
...
function car_scene () {
    car_geometry = new THREE.BoxGeometry( 8, 2, 2 );
    car_material = new THREE.MeshBasicMaterial( {color:0xD62D20, side: THREE.FrontSide} );
    cars1[0] = new THREE.Mesh( car_geometry, car_material );
    cars2[0].position.x = -30;
    cars1[0].position.y = -13;
    scene.add( cars1[0] );
    for( i=1; i<4; i++ ) {
        cars1[i] = new THREE.Mesh( car_geometry, car_material );
        cars1[i].position.y = -13;
        cars1[i].position.x = -30 + (i*10 + 15);
        scene.add(cars1[i])
    }
    for( i=0; i<4; i++ ) {
        cars2[i] = new THREE.Mesh( car_geometry, car_material );
        cars2[i].position.y = -5;
        cars2[i].position.x = -30 + (i*10 + 15);
        scene.add( cars2[i] );
    }
}

function log_scene () {
    log_geometry = new THREE.BoxGeometry( 8, 4, 2 );
    log_material = new THREE.MeshBasicMaterial( {color: 0x4B3832, side: THREE.FrontSide} );
    log_width = 8;
    log_height = 4;
    logs1[0] = new THREE.Mesh( log_geometry, log_material );
    logs1[0].position.x = -30;
    logs1[0].position.y = 10;
    scene.add(logs1[0]);

    for( i = 1; i < 4; i++ ) {
        logs1[i] = new THREE.Mesh( log_geometry, log_material );
        logs1[i].position.y = 10;
        logs1[i].position.x = -30 + ( i * 15 + 15 )
        scene.add( logs1[i] )
    }

    for( i = 0; i < 4; i++ ) {
        logs2[i] = new THREE.Mesh( log_geometry, log_material );
...
function death() {
    if( num_lives > 1 ) {
        ...
        audio.appendChild(source);
        audio.play();
        alert( 'Oops! You lost your life! You have '+num_lives+' lives left.' );
...
function move_object() {
    ...
    for( i = 0; i < logs2.length; i++ ) {
        if( logs2[i].position.x >= -40 ) logs2[i].position.x -= 0.1;
